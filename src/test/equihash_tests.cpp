                           ...
                           std::vector<unsigned char> expanded)
{
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(expanded.begin(), expanded.end(), out.begin(), out.end());

  out.resize(compact.size());
  CompressArray(expanded.data(), expanded.size(),
                out.data(), out.size(), bit_len, byte_pad);
  BOOST_CHECK_EQUAL_COLLECTIONS(compact.begin(), compact.end(), out.begin(), out.end());
}

BOOST_AUTO_TEST_CASE(expand_and_contract_arrays)
    {
...
{
  std::vector<eh_index> indices1 = GetIndicesFromMinimal(minimal, cBitLen);
  BOOST_CHECK_EQUAL_COLLECTIONS(indices.begin(), indices.end(), indices1.begin(), indices1.end());
  std::vector<unsigned char> minimal1 = GetMinimalFromIndices(indices, cBitLen);
  BOOST_CHECK_EQUAL_COLLECTIONS(minimal.begin(), minimal.end(), minimal1.begin(), minimal1.end());
}

BOOST_AUTO_TEST_CASE(minimal_solution_representation)
    {
...
BOOST_AUTO_TEST_CASE(is_probably_duplicate)
    {
        ...
        BOOST_CHECK(!IsProbablyDuplicate<4>(p1, 4));
        BOOST_CHECK(!IsProbablyDuplicate<4>(p2, 4));
        BOOST_CHECK(IsProbablyDuplicate<4>(p3, 4));
    }

#ifdef ENABLE_MINING
BOOST_AUTO_TEST_CASE(check_basic_solver_cancelled) {
    ...
    {
        BOOST_CHECK_NO_THROW(Eh48_5.BasicSolve(state, [](std::vector<unsigned char> soln) {
            return false;
    ...
    {
        BOOST_CHECK_THROW(Eh48_5.BasicSolve(state, [](std::vector<unsigned char> soln) {
            return false;
        }, [](EhSolverCancelCheck pos) {
...
BOOST_AUTO_TEST_CASE(check_optimised_solver_cancelled) {
    Equihash<48,5> Eh48_5;
    crypto_generichash_blake2b_state state;
    Eh48_5.InitialiseState(state);
    uint256 V = uint256S("0x00");
    ...
    {
        BOOST_CHECK_NO_THROW(Eh48_5.OptimisedSolve(state, [](std::vector<unsigned char> soln) {
            return false;
        }, [](EhSolverCancelCheck pos) {
            return false;
        }));
    ...
    {
        BOOST_CHECK_THROW(Eh48_5.OptimisedSolve(state, [](std::vector<unsigned char> soln) {
            return false;
        }, [](EhSolverCancelCheck pos) {
            return pos == ListGeneration;
        }), EhSolverCancelledException);
